...
contract Agent {
    ...
    function claimTokens(
        ...
        if (permitVersion == 0) {
            ERC20Permit0(address(token)).permit(
                owner,
                address(this),
                amount,
                deadline,
            ...
            ERC20Permit1(address(token)).permit(
                owner,
                address(this),
                nonce,
                deadline,
    ...
    function swapTokensForEth(address token, uint256 amount)
        internal
        returns (uint256)
    {
    ...
    function sendETHMinusFee(
        ...
        if (etherFee < ethReceived) {
            // shoud always be true
    ...
    function sendETHToReceiver(address payable receiver, uint256 amount)
        internal
        returns (bool success)
    {
